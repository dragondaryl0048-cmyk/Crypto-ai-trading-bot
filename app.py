...
from data_fetch import get_top_coins
...
def render_scan_table(rows: list[dict]):
    ...
    if df.empty:
        ...
        return
    display = df[["symbol", "price", "verdict", "confidence", "rsi", "target", "stop_loss", "risk_reward", "holding_period"]].copy()
    display.columns = ["Coin", "Price ($)", "Verdict", "Confidence (%)", "RSI", "Target ($)", "Stop-loss ($)", "R:R", "Holding period"]
    ...
    if mode == "Scan all coins":
        ...
        if st.button("🔄 Run scan", type="primary"):
            ...
            for i, row in coins_df.iterrows():
                ...
                time.sleep(0.3)  # be polite to the free API's rate limit
            progress.empty()
