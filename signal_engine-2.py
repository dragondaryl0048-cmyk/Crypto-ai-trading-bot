...
from data_fetch import get_ohlc, RateLimitError
from ml_predictor import train_and_predict


def analyze_coin(coin_id: str, symbol: str, days: int = 180) -> dict:
    try:
        df = get_ohlc(coin_id, days=days)
    except RateLimitError as e:
        return {"coin_id": coin_id, "symbol": symbol, "error": str(e)}
    ...
    # --- ML prediction (free, local, no API key) ---
